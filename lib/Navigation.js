...
var Navigation = Class.$extend({
  ...
  _buildNeighboursLookupTable: function (polygons) {

    for (var i = 0; i < polygons.length; i++) {
      var minIndex = Math.min(polygons[i].vertexIds[0], polygons[i].vertexIds[1], polygons[i].vertexIds[2]);
      var maxIndex = Math.max(polygons[i].vertexIds[0], polygons[i].vertexIds[1], polygons[i].vertexIds[2]);
      var midIndex = polygons[i].vertexIds[0] + polygons[i].vertexIds[1] + polygons[i].vertexIds[2] - minIndex - maxIndex;
      ...
      this.neighboursLookupTable[minIndex][midIndex].push(polygons[i]);
      this.neighboursLookupTable[minIndex][maxIndex].push(polygons[i]);
      this.neighboursLookupTable[midIndex][maxIndex].push(polygons[i]);

      this.polygonPositionInTable[minIndex][midIndex][polygons[i].id] = (this.neighboursLookupTable[minIndex][midIndex]).length;
      this.polygonPositionInTable[minIndex][maxIndex][polygons[i].id] = (this.neighboursLookupTable[minIndex][maxIndex]).length;
      this.polygonPositionInTable[midIndex][maxIndex][polygons[i].id] = (this.neighboursLookupTable[midIndex][maxIndex]).length;
